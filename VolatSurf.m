...
% grab first all the call and put options for a given maturity
    ...
    b0=[0.05; 0.1]; %set initial implied interest to 5% and dividend to 10%
...
% grab first all the call and put options for a given maturity
    ...
    y = C-P;
    NbOpt = rows(y);
    x = [ones(NbOpt,1)*S0, -K(1:NbStrik)]
    % use Kevin Sheppard routine
    const=0;
    [b, tstat, s2, vcv, vcvwhite, R2, Rbar, yhat] = ols(y,x,const);
    b1=b(1);
    b2=b(2);
    d=-log(b1)/Tv(i);
    r=-log(b2)/Tv(i);
...
M=100*[ShId ShIr];
fprintf('Implied dividends and interest rates via Shimko\n')
niceprint(M)
...
for i=1:NbOpt
    % invert option prices numerically
    % gets parameters
    S0 = AllInfo(i,1);
    K  = AllInfo(i,2);
    CPi= AllInfo(i,3);
    ...
    ub=[ 2 ]; %upper bounds for moments
    sig0=0.12; %set initial volatility to 12% Notice that starting values are crucial here 
    options=optimset('Diagnostics','on','Display','iter');
...
for i=1:NbMat
    ...
    ivc_im  = ImpVol(idx).*(1-e(idx)); %out of the money option
    ivc_all = ImpVol(idx);
    
    idx     = NbCall+NbStrik*(i-1)+1:NbCall+NbStrik*(i-1)+NbStrik; idx=idx';
    ivp_im  = ImpVol(idx).*(e(idx)); % out of the money put option
    ivp_all = ImpVol(idx);
